...
def main(j, args, params, tags, tasklet):
    ...
    if contentdir and space_path:
        ...
        if not any(not os.path.relpath(real_space_path, p).startswith('..') for p in permitted_dirs):
            page.addMessage(errmsg('The space path should only be under {}'.format(" or ".join(permitted_dirs))))
            return params

        if not re.search('^[/\w\d]+$', real_space_path):
            page.addMessage(errmsg('The space path should not contain any special characters'))
            return params

        if os.path.exists(space_path):
            page.addMessage(errmsg('The space path "{}" already exists'.format(space_path)))
            return params

        if not os.path.exists(contentdir):
            page.addMessage(errmsg('The content dir "{}" does not exist'.format(contentdir)))
            return params
        ...
        for directory in os.listdir(contentdir):
            ...
            if space_name == directory or\
               space_name == os.path.basename(os.readlink(os.path.join(contentdir, directory))):
                page.addMessage(errmsg('The space name "{0}" with contentdir "{1}" already exists'.format(space_name, contentdir)))
                return params
