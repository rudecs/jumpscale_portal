...
PUBKEY=`cat $PUBPATH`
echo "give $USER password: "
read -s  PASSWORD

curl -X POST -F "pubkey=$PUBKEY" -F "passwd=$PASSWORD" -F "login=$USER" -F "hostname=$HOSTNAME" http://%s:82/restmachine/system/cpunode/create | bash"""
        ctx = kwargs["ctx"]
        headers = [('Content-Type', 'text/plain'), ]
        ctx.start_response("200", headers)
        return (script % portalAddrr).strip()

    def create(self, login, passwd, pubkey, hostname, **kwargs):
        """
        ...
        cmd = "echo '%s' >> ~/.ssh/authorized_keys" % pubkey
        j.do.execute(cmd, outputStdout=False, outputStderr=False, useShell=True, captureout=True, dieOnNonZeroExitCode=True)
...
"""
    ...
    def _findFreePort(self):
        ...
        for p in sorted(free_port):
            if not j.system.net.tcpPortConnectionTest('127.0.0.1', p, timeout=1):
                return p
